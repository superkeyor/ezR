...
ez.countplot = function(df,cmd,position='both',color='color',colors=ez.palette("Zhu"),alpha=1,title.size=20,n.size=5.5,n.angle=0,n.type=3,width=0.7,ylab=NULL,xlab=NULL,zlab=NULL,legend.position='top',legend.direction="horizontal",legend.box=T,legend.size=c(0,10),xangle=0,vjust=NULL,hjust=NULL,facet='cols',theme.apa=TRUE) {
    ...
    if (length(cmd)==1) {
        ...
        gghistory=paste(gghistory,
                 ...
                 dfdf = dfdf %>% dplyr::count(c("theKey","theValue")) %>% dplyr::group_by("theKey") %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct)
                 # then compute n/pct
                 dfdf = dfdf %>% dplyr::count(c("theKey","theValue")) %>% dplyr::group_by("theKey") %>% dplyr::mutate(pct=n/sum(dfdf[["n"]]),pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))',
                 tt,sep='\n')
    # xx|zz or xx|zz aa
    } else {
        xx = cmd[1]

...
#' \cr         if it is already summary data, use mutate(tidyr::uncount(counter), value = 1) to turn into long format
...
#' @param reorder T/F whether to reorder x by the group means (raw means if no covar, adjusted means if covar)
#' @param reorder.direction 'az' or 'za'
...
#' @param na.rm If `FALSE`, removes missing values with
...
ez.barplot = function(df,cmd,color='color',colors=ez.palette("Zhu"),bar.gap=0.7,bar.width=0.7,error.size=0.7,error.gap=0.7,error.width=0.3,error.direction='both',ylimits=NULL,ybreaks=NULL,ylab=NULL,xlab=NULL,zlab=NULL,legend.position='top',legend.direction="horizontal",legend.box=T,legend.size=c(0,10),xangle=0,vjust=NULL,hjust=NULL,print2scr=TRUE,
    ...
    reorder=FALSE,reorder.direction='az',
...
ez.countplot = function(df,cmd,position='both',color='color',colors=ez.palette("Zhu"),alpha=1,title.size=20,n.size=5.5,n.angle=0,n.type=3,width=0.7,ylab=NULL,xlab=NULL,zlab=NULL,legend.position='top',legend.direction="horizontal",legend.box=T,legend.size=c(0,10),xangle=0,vjust=NULL,hjust=NULL,facet='cols',theme.apa=TRUE) {
    ...
    if (length(cmd)==1) {
        ...
        xx = strsplit(xx,"/",fixed=TRUE)[[1]]
        df = ez.dropna(df, xx)
        dfdf = df %>% tidyr::gather(!!"theKey",!!"theValue",!!rlang::sym(xx))
        # keep factor order
        if (position=="fill") {
        ...
        # first compute pos
        dfdf = dfdf %>% dplyr::count(!!"theKey",!!"theValue") %>% dplyr::group_by(!!"theKey") %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct)
        # then compute n/pct
        ...
        if (position=='stack') {
            ...
            pp = ggplot2::ggplot(dfdf, aes(x=%s,n,fill=%s)) +
                         ...
                         , 'theKey','theValue', position, alpha, width, color, ylab, xlab, zlab, legend.position, legend.box, title.size, xangle, vjust, hjust, legend.direction, legend.size[1], legend.size[2], legend.size[2], n.size, n.angle, n.type.stack
            )
            ...
            pp = ggplot2::ggplot(dfdf, aes(x=%s,n,fill=%s)) +
                         ...
                         , 'theKey','theValue', position, alpha, width, color, ylab, xlab, zlab, legend.position, legend.box, title.size, xangle, vjust, hjust, legend.direction, legend.size[1], legend.size[2], legend.size[2], n.size, n.angle, n.type.fill
            )
        }
        gghistory=paste(gghistory,
                 ...
                 dfdf = dfdf %>% dplyr::count(theKey,theValue) %>% dplyr::group_by(theKey) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct)
                 # then compute n/pct
                 dfdf = dfdf %>% dplyr::count(theKey,theValue) %>% dplyr::group_by(theKey) %>% dplyr::mutate(pct=n/sum(dfdf[["n"]]),pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))',
                 tt,sep='\n')
    # xx|zz or xx|zz aa
    } else {
        xx = cmd[1]
        ...
        if (length(zz)==1) {
            ...
            dfdf = df %>% dplyr::count(!!rlang::sym(xx),!!rlang::sym(zz)) %>% dplyr::group_by(!!rlang::sym(xx)) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct,pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))
            if (position=='stack') {
                if (is.null(ylab)) ylab='Count'
                ylab = ifelse(is.null(ylab),'',sprintf('ylab("%s")+',ylab))
                tt = sprintf('
            ...
            gghistory=paste(gghistory,
                     ...
                     sprintf('xx="%s"',xx),
                     sprintf('zz="%s"',zz),
                     'dfdf = df %>% dplyr::count(xx,zz) %>% dplyr::group_by(xx) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct,pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))',
                     tt,sep='\n')
        # xx|zz aa
        } else {
            if (length(zz)==2) {
                ...
                if (position=='stack') {
                    ...
                    pp = ggplot2::ggplot(dfdf, aes(x=%s,n,fill=%s)) +
                                 ...
                                 , xx, zz, position, alpha, width, color, ylab, xlab, zlab, legend.position, legend.box, title.size, xangle, vjust, hjust, legend.direction, legend.size[1], legend.size[2], legend.size[2], n.size, n.angle, n.type.stack, sprintf(ifelse(facet=="cols","facet_grid(.~%s)",ifelse(facet=="rows","facet_grid(%s~.)","facet_wrap(~%s)")),aa)
                    )
                } else if (position=='fill') {
                    if (is.null(ylab)) ylab='Percentage'
                    ylab = ifelse(is.null(ylab),'',sprintf('ylab("%s")+',ylab))
                    ...
                    pp = ggplot2::ggplot(dfdf, aes(x=%s,n,fill=%s)) +
                                 ...
                                 scale_y_continuous(labels=scales::percent)+coord_flip()'
                                 , xx, zz, position, alpha, width, color, ylab, xlab, zlab, legend.position, legend.box, title.size, xangle, vjust, hjust, legend.direction, legend.size[1], legend.size[2], legend.size[2], n.size, n.angle, n.type.fill, sprintf(ifelse(facet=="cols","facet_grid(.~%s)",ifelse(facet=="rows","facet_grid(%s~.)","facet_wrap(~%s)")),aa)
                    )
                }
            gghistory=paste(gghistory,
                     ...
                     sprintf('zz="%s"',zz),
                     sprintf('aa="%s"',aa),
                     'dfdf = df %>% dplyr::count(xx,zz,aa) %>% dplyr::group_by(aa,xx) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct,pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))',
                     tt,sep='\n')
            }
        }
    }
...
ez.piechart = function(df,cmd,start=0,direction=1,color='color',colors=ez.palette("Zhu"),alpha=1,n.size=5.5,n.type=3,n.angle=0,ylab='',xlab='',zlab=NULL,legend.position='top',legend.direction="horizontal",legend.box=T,legend.size=c(0,10),xangle=0,vjust=NULL,hjust=NULL,theme.apa=TRUE) {
    ...
    if (ylab=='') ylab=xx
    df = ez.dropna(df, xx)
    dfdf = df %>% dplyr::count(!!rlang::sym(xx)) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct,pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))
    ...
    gghistory=paste(gghistory,
         ...
         sprintf('xx="%s"',xx),
         'dfdf = df %>% dplyr::count(xx) %>% dplyr::mutate(pct=n/sum(n),pct.pos=cumsum(n)-0.5*n,n.pos=cumsum(pct)-0.5*pct,pct.str=sprintf("%0.1f%%",pct*100),n.str=sprintf("(%d)",n),n.pct.str=sprintf("%d (%0.1f%%)",n,pct*100),pct.n.str=sprintf("%0.1f%% (%d)",pct*100,n))',
         tt,sep='\n')
    if (theme.apa) tt = paste0(tt,'+theme_apa()')
    eval(parse(text = tt))
    pp$gghistory=paste0(gghistory,'\nprint(pp)')
